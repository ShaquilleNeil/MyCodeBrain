...
# import schedule to schedule the script to run every start and end of day
import schedule

def get_account_details(login, password, server):
    #start the MetaTrader 5 engine
    mt5.initialize()

    #Login to MetaTrader 5
    mt5.login(int(login), password, server) # login has to be integer otherwise it won't work
    ...
    return {
        ...
        "Equity": info.equity,
        "Profit": info.profit,
        "Free Margin": info.margin_free
    }
  
  
    # loop
for row in range(df_credentials.shape[0]):
        data = get_account_details(row["Login"], row["Password"], row["Server"])
        print(data)
